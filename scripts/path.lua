...
function M.Advance(moveDist)
    ...
    -- 注意：turnJustCommitted 由外部（main.lua）在转弯处理后清除，不在此处清除
...
function M.EnterIntersection(overshoot)
    ...
    -- 地图边缘可能直行无路，但左/右仍可走；进入路口时不要误判为死路。
    local exitEdge = rn.GetEdgeByHeading(targetNodeId, s.intersectionExitHeading)
    if not exitEdge and #s.availableTurns == 0 then
    ...
    -- 记录到达方向（供外部转弯处理使用，不在此处触发 turnJustCommitted）
